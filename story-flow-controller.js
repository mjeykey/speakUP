(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .speakup-flow-overlay{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:28px;background:radial-gradient(circle at 20% 20%,rgba(255,95,215,.14),transparent 36%),radial-gradient(circle at 80% 80%,rgba(101,232,255,.13),transparent 38%),#020205;color:#fff;opacity:1;transition:opacity .45s ease}.speakup-flow-overlay.leaving{opacity:0}.speakup-flow-card{width:min(1080px,100%);text-align:center}.speakup-flow-label{font-family:Arial,sans-serif;font-size:14px;letter-spacing:.16em;text-transform:uppercase;color:#9daac7;margin-bottom:24px}.speakup-flow-text{font-family:Georgia,'Times New Roman',serif;font-size:clamp(30px,5vw,64px);line-height:1.5;font-style:italic;text-wrap:balance}.speakup-flow-word{display:inline-block;padding:0 .08em;border-radius:.2em;transition:color .13s ease,background .13s ease,transform .13s ease}.speakup-flow-word.active{color:#020205;background:#65e8ff;transform:translateY(-1px) scale(1.035)}.speakup-flow-hidden{visibility:hidden!important;pointer-events:none!important}.speakup-flow-countdown{margin-top:24px;font:13px Arial,sans-serif;letter-spacing:.08em;color:#7f8ba6;min-height:18px}.speakup-flow-start{margin-top:28px;border:1px solid rgba(101,232,255,.65);background:rgba(101,232,255,.12);color:#fff;border-radius:999px;padding:14px 24px;font:16px Arial,sans-serif}.speakup-flow-start:disabled{opacity:.55}`;
  ...
  function createOverlay(label, text, needsStartButton = false) {
    ...
    if (startButton) { startButton.type = 'button'; startButton.className = 'speakup-flow-start'; startButton.textContent = 'Audio starten'; card.append(heading, line, startButton, countdown); }
    else card.append(heading, line, countdown);
  ...
  function speakHighlighted(text, lang, rate, wordElements) {
    return new Promise(resolve => {
      ...
      (window.__speakUpCancelSpeech || synth.cancel.bind(synth))();
      const utterance = new SpeechSynthesisUtterance(value);
      utterance.lang = lang; utterance.rate = rate; utterance.pitch = 1; utterance.volume = 1; utterance.__speakUpFlowPreview = true;
      const voice = pickVoice(lang); if (voice) utterance.voice = voice;
      const tokens = tokenize(value);
      let activeIndex = -1, fallbackTimer = null, watchdog = null, started = false, boundaryReceived = false, finished = false;
      const activate = index => { if (index < 0 || index >= wordElements.length || index === activeIndex) return; if (activeIndex >= 0) wordElements[activeIndex]?.classList.remove('active'); activeIndex = index; wordElements[index]?.classList.add('active'); };
      const cleanup = () => { if (finished) return; finished = true; if (fallbackTimer) clearInterval(fallbackTimer); if (watchdog) clearTimeout(watchdog); if (activeIndex >= 0) wordElements[activeIndex]?.classList.remove('active'); resolve(); };
      utterance.onstart = () => {
        started = true; activate(0);
        const estimatedTotal = Math.max(1800, tokens.length * (lang.startsWith('pt') ? 650 : 430) / Math.max(rate, .5));
        let fallbackIndex = 0;
        fallbackTimer = window.setInterval(() => { if (boundaryReceived) return; fallbackIndex += 1; if (fallbackIndex < wordElements.length) activate(fallbackIndex); }, estimatedTotal / Math.max(tokens.length, 1));
      };
      ...
      utterance.onend = cleanup; utterance.onerror = cleanup;
      watchdog = window.setTimeout(() => { if (!started) { (window.__speakUpCancelSpeech || synth.cancel.bind(synth))(); cleanup(); } }, 5000);
      const directSpeak = window.__speakUpSpeakPreview || synth.speak.bind(synth);
      directSpeak(utterance);
    });
  }
  ...
  async function runPreview(storyText) {
    ...
    await waitForAudioUnlock(preview); await delay(TIMING.openingPause);
    ...
    preview = createOverlay('Ouve em português', portuguese); await delay(TIMING.openingPause);
  ...
  document.addEventListener('click', event => { const button = event.target instanceof Element ? event.target.closest('button') : null; if (button && /menu|back/i.test(cleanText(button.textContent))) (window.__speakUpCancelSpeech || synth?.cancel?.bind(synth))?.(); }, true);
  scan();
})();