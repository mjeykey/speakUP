...
export function renderMemory(root, store) {
  ...
  const showMessage = async item => {
    ...
    await speak(learningSentence, memorySpeechLanguage(learningLanguage), { enabled: store.getState().audioOn, rate: learningLanguage.startsWith('pt') || learningLanguage.startsWith('fr') ? .62 : .78 });
    await sleep(500);
    ...
    await speak(nativeSentence, memorySpeechLanguage(nativeLanguage), { enabled: store.getState().audioOn, rate: nativeLanguage.startsWith('pt') || nativeLanguage.startsWith('fr') ? .62 : .86 });
    await sleep(500);
    await dissolve(stage.querySelector('[data-message-text]'));
    stage.innerHTML = '';
    locked = false;
