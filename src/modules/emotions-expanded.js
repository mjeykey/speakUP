...
export function renderEmotions(root,store){
 ...
 const wireSpeech=()=>{
   root.querySelectorAll('[data-learning-say]').forEach(el=>el.onclick=()=>sayLearning(el.dataset.learningSay));
   root.querySelectorAll('[data-support-say]').forEach(el=>el.onclick=event=>{event.preventDefault();event.stopPropagation();saySupport(el.dataset.supportSay);});
 };
