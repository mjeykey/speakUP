...
export function renderEmotions(root,store){
 const state=store.getState();const ui=getExerciseUiCopy(state.nativeLanguage);const code=canonical(state.learningLanguage);const supportCode=canonical(state.nativeLanguage);const items=DATA[code]||DATA['en-GB'];const supportItems=DATA[supportCode]||DATA['en-GB'];const voice=getSpeechLanguage(state.learningLanguage);const supportLanguageLabel=supportLabel(supportCode);let selected=null;let selectedIndex=0;let quizIndex=0;let feedback='';let animating=false;
 const say=t=>speak(t,voice,{enabled:state.audioOn,rate:.72}).catch(()=>{});const leave=()=>{stopSpeech();store.setState({screen:'menu'});};
 function picker(){root.innerHTML=`<section class="screen emotions-screen"><button class="menu-button" data-menu>${ui.menu}</button><div class="emotions-shell"><p class="kicker">${esc(languageName(state.learningLanguage))}</p><h1>${ui.emotions}</h1><div class="emotion-grid">${items.map((x,i)=>`<button class="emotion-card" data-i="${i}"><span>${x[0]}</span><strong>${esc(x[1])}</strong><small class="emotion-card-support">${esc(supportItems[i]?.[1]||'')}</small></button>`).join('')}</div></div></section>`;root.querySelector('[data-menu]').onclick=leave;root.querySelectorAll('[data-i]').forEach(b=>b.onclick=()=>{selectedIndex=Number(b.dataset.i);selected=items[selectedIndex];quizIndex=0;feedback='';detail();});}
 function detail(){animating=false;const sentence=selected[3][quizIndex];const supportSentence=supportItems[selectedIndex]?.[3]?.[quizIndex]||'';const words=sentence.replace(/[.,!?]/g,'').split(/\s+/);const answer=words[Math.min(1,words.length-1)];const gap=sentence.replace(answer,'___');const distractors=selected[2].filter(x=>x.toLowerCase()!==answer.toLowerCase()).slice(0,2);const options=[answer,...distractors].sort(()=>Math.random()-.5);const optionSupport=x=>{const descriptorIndex=selected[2].findIndex(word=>word.toLowerCase()===String(x).toLowerCase());if(descriptorIndex>=0)return supportItems[selectedIndex]?.[2]?.[descriptorIndex]||'';if(code==='en-GB'&&supportCode==='pt-PT')return EN_PT_QUIZ_HINTS[String(x).toLowerCase()]||'';return'';};root.innerHTML=`<section class="screen emotions-screen"><button class="menu-button" data-menu>${ui.menu}</button><div class="emotions-shell emotion-journey"><div class="emotion-current-block"><p class="emotion-current">${selected[0]} ${esc(selected[1])}</p><p class="translation emotion-current-translation"><span>${esc(supportLanguageLabel)}:</span> ${esc(supportItems[selectedIndex]?.[1]||'')}</p></div><div class="emotion-panel"><p class="kicker">${ui.words}</p><div class="emotion-answer-grid">${selected[2].map((x,i)=>`<button class="emotion-answer emotion-bilingual-word" data-say="${esc(x)}"><span>${esc(x)}</span><small class="emotion-word-support">${esc(supportItems[selectedIndex]?.[2]?.[i]||'')}</small></button>`).join('')}</div></div><div class="emotion-panel"><p class="kicker">${ui.repeat}</p><button class="menu-card emotion-repeat-card" data-say="${esc(sentence)}"><span>${esc(sentence)}</span><small class="emotion-support-sentence"><span>${esc(supportLanguageLabel)}:</span> ${esc(supportSentence)}</small><small>🔊 ${ui.tapRepeat}</small></button></div><div class="emotion-panel"><p class="kicker">${ui.miniExercise}</p><p class="story-copy" data-emotion-gap-sentence>${esc(gap)}</p><p class="emotion-support-sentence emotion-gap-support"><span>${esc(supportLanguageLabel)}:</span> ${esc(supportSentence)}</p><div class="emotion-answer-grid">${options.map(x=>`<button class="emotion-answer emotion-bilingual-word" data-answer="${esc(x)}"><span>${esc(x)}</span>${optionSupport(x)?`<small class="emotion-word-support">${esc(optionSupport(x))}</small>`:''}</button>`).join('')}</div><p class="feedback" data-emotion-feedback>${esc(feedback)}</p></div><div class="memory-actions"><button class="secondary-button" data-back>← ${ui.emotions}</button><button class="primary-button" data-next>${ui.nextSentence}</button></div></div></section>`;root.querySelector('[data-menu]').onclick=leave;root.querySelector('[data-back]').onclick=picker;root.querySelectorAll('[data-say]').forEach(b=>b.onclick=()=>say(b.dataset.say));root.querySelectorAll('[data-answer]').forEach(b=>b.onclick=async()=>{if(animating)return;if(b.dataset.answer!==answer){feedback=ui.tryAgain;detail();return;}animating=true;feedback=`✓ ${ui.correct}`;const sentenceEl=root.querySelector('[data-emotion-gap-sentence]');const feedbackEl=root.querySelector('[data-emotion-feedback]');const nextButton=root.querySelector('[data-next]');root.querySelectorAll('[data-answer]').forEach(button=>button.disabled=true);if(nextButton)nextButton.disabled=true;const effect=getModeTextEffect('emotions');if(sentenceEl){sentenceEl.innerHTML=completedSentenceHtml(sentence,answer);sentenceEl.dataset.effect=effect;}if(feedbackEl)feedbackEl.textContent=feedback;await new Promise(resolve=>window.setTimeout(resolve,900));if(sentenceEl){try{await explodeText(sentenceEl,effect,{duration:1450,stagger:20});}catch(error){console.warn('Emotion sentence effect failed.',error);}}quizIndex=(quizIndex+1)%selected[3].length;feedback='';detail();});root.querySelector('[data-next]').onclick=()=>{if(animating)return;quizIndex=(quizIndex+1)%selected[3].length;feedback='';detail();};}
 picker();
}
