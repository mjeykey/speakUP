...
export function renderEmotions(root,store){
 const state=store.getState();const ui=getExerciseUiCopy(state.nativeLanguage);const code=canonical(state.learningLanguage);const supportCode=canonical(state.nativeLanguage);const items=DATA[code]||DATA['en-GB'];const supportItems=DATA[supportCode]||DATA['en-GB'];const voice=getSpeechLanguage(state.learningLanguage);let selected=null;let selectedIndex=0;let quizIndex=0;let feedback='';
 const say=t=>speak(t,voice,{enabled:state.audioOn,rate:.72}).catch(()=>{});const leave=()=>{stopSpeech();store.setState({screen:'menu'});};
 function picker(){root.innerHTML=`<section class="screen emotions-screen"><button class="menu-button" data-menu>${ui.menu}</button><div class="emotions-shell"><p class="kicker">${esc(languageName(state.learningLanguage))}</p><h1>${ui.emotions}</h1><div class="emotion-grid">${items.map((x,i)=>`<button class="emotion-card" data-i="${i}"><span>${x[0]}</span><strong>${esc(x[1])}</strong></button>`).join('')}</div></div></section>`;root.querySelector('[data-menu]').onclick=leave;root.querySelectorAll('[data-i]').forEach(b=>b.onclick=()=>{selectedIndex=Number(b.dataset.i);selected=items[selectedIndex];quizIndex=0;feedback='';detail();});}
 function detail(){const sentence=selected[3][quizIndex];const supportSentence=supportItems[selectedIndex]?.[3]?.[quizIndex]||'';const words=sentence.replace(/[.,!?]/g,'').split(/\s+/);const answer=words[Math.min(1,words.length-1)];const gap=sentence.replace(answer,'___');const distractors=selected[2].filter(x=>x.toLowerCase()!==answer.toLowerCase()).slice(0,2);const options=[answer,...distractors].sort(()=>Math.random()-.5);root.innerHTML=`<section class="screen emotions-screen"><button class="menu-button" data-menu>${ui.menu}</button><div class="emotions-shell emotion-journey"><p class="emotion-current">${selected[0]} ${esc(selected[1])}</p><p class="translation">${esc(supportItems[selectedIndex]?.[1]||'')}</p><div class="emotion-panel"><p class="kicker">${ui.words}</p><div class="emotion-answer-grid">${selected[2].map(x=>`<button class="emotion-answer" data-say="${esc(x)}">${esc(x)}</button>`).join('')}</div></div><div class="emotion-panel"><p class="kicker">${ui.repeat}</p><button class="menu-card" data-say="${esc(sentence)}"><span>${esc(sentence)}</span><small>${esc(supportSentence)}</small><small>🔊 ${ui.tapRepeat}</small></button></div><div class="emotion-panel"><p class="kicker">${ui.miniExercise}</p><p class="story-copy">${esc(gap)}</p><div class="emotion-answer-grid">${options.map(x=>`<button class="emotion-answer" data-answer="${esc(x)}">${esc(x)}</button>`).join('')}</div><p class="feedback">${esc(feedback)}</p></div><div class="memory-actions"><button class="secondary-button" data-back>← ${ui.emotions}</button><button class="primary-button" data-next>${ui.nextSentence}</button></div></div></section>`;root.querySelector('[data-menu]').onclick=leave;root.querySelector('[data-back]').onclick=picker;root.querySelectorAll('[data-say]').forEach(b=>b.onclick=()=>say(b.dataset.say));root.querySelectorAll('[data-answer]').forEach(b=>b.onclick=()=>{feedback=b.dataset.answer===answer?`✓ ${ui.correct}`:ui.tryAgain;detail();});root.querySelector('[data-next]').onclick=()=>{quizIndex=(quizIndex+1)%selected[3].length;feedback='';detail();};}
 picker();
}
