...
export function renderSpeakPractice(root,store){
  ...
  let recognition=null,listening=false,transcript='',heardAttempt='',message='',pronunciationText='',recognitionTimer=null;
  ...
  function showTopics(){
    clearRecognitionTimer();recognition?.abort?.();stopSpeech();save();topic=null;transcript='';heardAttempt='';message='';pronunciationText='';
  ...
  function draw(){
    ...
    const pronunciationBlock=pronunciationText?`<div class="free-speak-result free-speak-pronunciation-card"><p class="speak-label">${esc(copy.recommended)}</p><p class="free-speak-example">${esc(pronunciationText)}</p><button class="secondary-button" data-pronunciation>🔊 ${esc(copy.hearAgain)}</button></div>`:'';
    ...
    root.querySelector('[data-next]')?.addEventListener('click',()=>{stopSpeech();index=(index+1)%topic.turns.length;transcript='';heardAttempt='';message='';pronunciationText='';save();draw();window.setTimeout(playQuestion,220);});
  ...
  function startListening(){
    if(listening||!Recognition)return;stopSpeech();clearRecognitionTimer();pronunciationText='';recognition=new Recognition();recognition.lang=speechLanguage;recognition.interimResults=false;recognition.maxAlternatives=3;recognition.continuous=false;listening=true;message=copy.listening;draw();
