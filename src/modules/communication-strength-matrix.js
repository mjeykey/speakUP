...
export function renderCommunicationStrength(root, store) {
  ...
  function draw() {
    ...
    root.innerHTML = `<section class="screen communication-strength-screen"><button class="menu-button" data-menu>${ui.menu}</button><div class="center communication-strength-view">
      <p class="kicker">${ui.communication} · ${index + 1} / ${items.length}</p>
      <h1>${ui.sayItBetter}</h1>
      <div class="communication-card communication-original"><p class="communication-target">${esc(item.weak)}</p><p class="communication-translation">${esc(item.weakTranslation)}</p><button class="secondary-button" data-listen-original>🔊 ${ui.listen}</button></div>
      ${revealed ? `<p class="communication-instead">${ui.sayInstead}</p><div class="communication-card communication-strong"><p class="communication-target">${esc(item.strong)}</p><p class="communication-translation">${esc(item.strongTranslation)}</p><button class="secondary-button" data-listen-strong>🔊 ${ui.listen}</button></div><div class="communication-actions"><button class="secondary-button" data-prev ${index === 0 ? 'disabled' : ''}>←</button><button class="primary-button" data-next>${index === items.length - 1 ? ui.again : ui.next + ' →'}</button></div>` : `<button class="primary-button" data-reveal>${ui.sayInstead} →</button>`}
    </div></section>`;
