...
import { isStorySfxPlaying, preloadStorySfx, playStorySfx, stopStorySfx } from '../audio/story-sfx.js?v=36';
...
const TSAR_BELL_URL = new URL('../../assets/audio/soundreality-tsar-bell-sound-simulation-292699.mp3?v=969c6cd2-direct3', import.meta.url).href;

let tsarBellAudio = null;
...
function getStory(storyId, learningLanguage, nativeLanguage) {
  ...
  return {
    ...
    pages: fantasyStory.pages.map((source, index) => {
      ...
      // Page 2 keeps its separate warning bell. Pages 5-8 use ONLY the uploaded Tsar MP3.
      const sound = index === 1 ? 'warning-bell' : CHURCH_BELL_PAGES.has(index) ? 'tsar-bell' : source.sound;
...
function gapHtml(page, solved) {
  ...
  page.items.forEach((item, index) => {
    const replacement = index < solved
      ? `<span class="story-gap-solved">${escapeHtml(item.answer)}</span>`
      : `<span class="story-gap-english">${escapeHtml(item.hint)}</span>`;
    html = html.replace(
      new RegExp(`\\b${item.answer.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i'),
      replacement
    );
  });
  return html;
...
function ensureTsarBellAudio() {
  if (tsarBellAudio && tsarBellAudio.isConnected) return tsarBellAudio;
  const audio = document.createElement('audio');
  audio.setAttribute('playsinline', '');
  audio.preload = 'auto';
  audio.loop = false;
  audio.volume = 0.90;
  audio.src = TSAR_BELL_URL;
...
function isTsarBellPlaying() {
  return Boolean(tsarBellAudio && !tsarBellAudio.paused && !tsarBellAudio.ended && !tsarBellAudio.muted);
}

function stopTsarBell() {
  const audio = tsarBellAudio;
  if (!audio) return;
  audio.onended = null;
  audio.onerror = null;
...
function playTsarBell() {
  const audio = ensureTsarBellAudio();
  try {
    audio.pause();
    audio.currentTime = 0;
    audio.muted = false;
    audio.volume = 0.90;
    if (audio.src !== TSAR_BELL_URL) {
      audio.src = TSAR_BELL_URL;
      audio.load();
    }
    return Promise.resolve(audio.play()).catch(error => {
      console.warn('Direct uploaded Tsar bell playback failed.', error);
      return false;
    });
  } catch (error) {
    console.warn('Direct uploaded Tsar bell playback failed.', error);
...
export function renderStory(root, store) {
  ...
  const saveProgress = () => store.updateProgress('story', progressKey, {
    storyId,
  ...
  function ensureAmbience(current, audioEnabled, token) {
    ...
    // Hard isolation: pages 5-8 NEVER enter Story SFX bell/warning-bell code.
    if (current.sound === 'tsar-bell') {
      if (isTsarBellPlaying()) return;
    ...
    const start = () => {
      ...
      const volume = current.sound === 'rain' ? 0.40 : current.sound === 'warning-bell' ? 0.55 : 0.30;
      void playStorySfx(current.sound, { enabled: true, loop: current.sound === 'rain', volume });
    };
    if (current.sound === 'rain' || current.sound === 'warning-bell') {
      start();
      return;
    }
  ...
  async function renderPhase() {
    ...
    if (phaseIndex === 0) {
      ...
      if (current.sound !== 'rain' && current.sound !== 'tsar-bell') stopAmbience();
  ...
  function startBellForCurrentPage() {
    if (storyId !== 'fantasy-1' || !Boolean(store.getState().audioOn) || page().sound !== 'tsar-bell') return;
    void playTsarBell();
  }
  ...
  function navigate(direction) {
    ...
    const keepAmbience = currentSound === 'rain' || currentSound === 'tsar-bell';

    if (direction > 0) {
      if (phaseIndex === 2 && solved < page().items.length) {
  ...
  // Preload only the exact uploaded Tsar file. It is never substituted with a generated tone.
  if (storyId === 'fantasy-1') ensureTsarBellAudio();
  renderPhase();
}
