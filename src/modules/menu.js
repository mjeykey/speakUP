...
export function renderMenu(root, store) {
  ...
  MODES.forEach(([id, title, description]) => {
    ...
    button.onclick = () => {
      if (id === 'fill-gap') {
        store.setState({ mode: 'fill-gap', screen: 'sentence-level-select', currentIndex: 0 });
        return;
      }
      store.setState(current => ({ mode: id, selectedStory: current.selectedStory, currentIndex: 0 }));
    };
