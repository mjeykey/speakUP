...
const LEVELS = [
  ['l1', 'L1 · Language', 'Your classic SpeakUP language practice.'],
...
export function renderMenu(root, store) {
  ...
  root.innerHTML = `<section class="screen menu-screen"><div class="menu-panel">
    <h1>Choose your path</h1><p class="muted">Learn the language — or learn something through the language.</p>
  ...
  root.querySelectorAll('[data-l2-topic]').forEach(button => {
    button.onclick = () => store.setState({ learningLevel: 'l2', mode: 'l2-learning', selectedL2Topic: button.dataset.l2Topic, currentIndex: 0 });
  });

  root.querySelectorAll('[data-l3-topic]').forEach(button => {
    button.onclick = () => store.setState({ learningLevel: 'l3', mode: 'l3-learning', selectedL3Topic: button.dataset.l3Topic, currentIndex: 0 });
  });
